...
\section{Profiler}
In order to discover CPU activity variation and memory allocation for each step of the code, we used a profiler to display the events occurring in the application. The one chosen to discover what was needed was {\itshape Instruments} (formerly known as {\itshape Xray}), for MAC OS.
...
\begin{figure}[t]
    \centering
    \includegraphics[scale=0.4]{Screenshot_2021-06-03_at_17.00.42.png}
...
{\itshape OpenMP} is a parallel programming model for multiprocessor architectures, providing a portable and scalable model for developers of shared memory parallel applications. It allows programmers to set the maximum number of threads a program can create to run in parallel within the bit of code that is marked accordingly, running that section independently. At the end, the threads then join back into the primary one, continuing to the end of the program. 

\section{Parallelized Code}

In this section we will talk about the parts of the code present in the .omp file, referencing the sections by the name of the method and the number of the section in that method (if there is one, if not it will be specified by the line number).
