...
Ao comparamos estas três versões em tempos de execução, conseguimos que a versão 4.2 era ligeiramente mais rápida. decidindo inicialmente ficar com esta, no entanto, após correr alguns testes com nº de threads diferentes chegamos à conclusão de que esta não era a melhor solução visto devolver alguns valores errados. Isto poderá dever se ao facto de a função atomic, apenas conseguir realizar operações atomicas para atualizar variaveis. O que poderá ter entrado em conflito na verificação para determinar  valor maximo dos valores. Assim decidimos usar a função critical, que também permite sincronizar o acesso a blocos por diferente threads. corrigindo os nossos erros de accuracy.
