...
The usage of {\itshape OpenMP's} atomic operation allows a multiple threads to update a shared numeric variable.
...
Using {\itshape OpenMP's} critical function allows to set a section of the code as critical, meaning that only one thread at a time can enter that bit. Only when a thread is finished running the delimited section, can the next thread run it.
\\Compared to the atomic implementation, by being able to syncronize the access to blocks by different threads, the accuracy errors discovered above mentioned were solved, at the cost of decreased performance.
...
\\(FALAR GRÁFICO)
\\With the values presented above, we can conclude that, as we increase the number of threads, the execution time of our program was decreasing slightly. We can observe this phenomenon happen on up to sixteen threads. After this number, the execution time starts rising, being the performance cap at sixteen threads.
